...
int add_location(int lid) {
	int i, newPos, lastPos;

	for (lastPos = 0; lastPos < MAX_LOCATIONS && locations_array[lastPos]->lid != 0; lastPos++);
	for (newPos = 0; newPos < MAX_LOCATIONS && locations_array[lastPos]->lid != 0 && locations_array[newPos]->lid < lid; newPos++);
	newPos--;

	if (lastPos >= (MAX_LOCATIONS - 1)) {
		fprintf(stderr, "Location list is full!");
		return 0;
	}

	/* Search the list for the next bigger lid and shove it */
	for (; lastPos > newPos; lastPos--) {
		locations_array[lastPos]->lid = locations_array[lastPos - 1]->lid;
		locations_array[lastPos]->poi_tree = locations_array[lastPos - 1]->poi_tree;
		locations_array[lastPos]->poi_sentinel = locations_array[lastPos - 1]->poi_sentinel;
	}

	locations_array[newPos]->lid = lid;

	printf("L %d\n\tLocations = ", lid);
	for (i = 0; i < MAX_LOCATIONS && locations_array[i]->lid != 0; i++) {
		printf("%d", locations_array[i]->lid);
		if (locations_array[i + 1]->lid)
			printf(", ");
	}
	printf("\nDONE\n");
